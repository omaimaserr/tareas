...
class ModeloPeliDB
{
    ...
    private function __construct()
    {
        ...
        try {
            ...
            $this->stmt_update  = $this->dbh->prepare("UPDATE peliculas set 
                nombre=:nombre, 
                director=:director, 
                genero=:genero, 
                imagen=:imagen, 
            ...
            $this->stmt_usuario = $this->dbh->prepare("SELECT * FROM usuarios WHERE usuario = :usuario");
        } catch (PDOException $e) {
    ...
    public function update($peli)
    {
        ...
        $this->stmt_update->bindValue(':trailer', $peli->trailer);
        $this->stmt_update->bindValue(':votos', $peli->votos);
        $this->stmt_update->bindValue(':puntuacion_total', $peli->puntuacion_total);
    ...
    {
        $stmt = $this->dbh->prepare("SELECT * FROM peliculas WHERE nombre LIKE :titulo");
    ...
    public function getByDirector($director)
    {
        $stmt = $this->dbh->prepare("SELECT * FROM peliculas WHERE director LIKE :director");
        $stmt->bindValue(':director', '%' . $director . '%');  // Usamos % para buscar coincidencias parciales
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_CLASS, 'Pelicula');
        $peliculas = $stmt->fetchAll();
        return $peliculas;
    ...
    {
        $stmt = $this->dbh->prepare("SELECT * FROM peliculas WHERE genero LIKE :genero");
        $stmt->bindValue(':genero', '%' . $genero . '%');  // Usamos % para buscar coincidencias parciales
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_CLASS, 'Pelicula');
        $peliculas = $stmt->fetchAll();
        return $peliculas;
    ...
    {
        $sql = "DELETE FROM peliculas WHERE codigo_pelicula = :id";
    ...
    public function getUsuarioByNombre($usuario) {
        $this->stmt_usuario->bindValue(':usuario', $usuario);
        $this->stmt_usuario->execute();
        $this->stmt_usuario->setFetchMode(PDO::FETCH_OBJ);
        return $this->stmt_usuario->fetch();
    }
